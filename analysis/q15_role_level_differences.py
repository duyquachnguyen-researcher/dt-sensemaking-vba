"""Analyze statement differences by role level for Q15."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Q15 role level difference analysis")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("outputs/q15_statement_role_level_gaps.csv"),
...
def summarize(stats: list[dict[str, Any]], top_n: int) -> str:
    lines = [
        ...
        "",
        "Top role-level-separated statements",
        "------------------------------------",
    ]
...
def main() -> None:
    ...
    group_map = load_group_map(args.participants, "role_level")
    ...
    sanity_lines = [
        ...
        f"Statements with >=2 role levels: {len(stats)}",
    ]
