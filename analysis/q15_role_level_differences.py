"""Analyze placement differences by participant role level for Q15."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Q15 placement differences by participant role level")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("outputs/q15_role_level_differences.csv"),
        help="Output CSV path",
    )
    parser.add_argument(
        "--output-summary",
        type=Path,
        default=Path("outputs/q15_summary.txt"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Minimum placements per role level to include in tests",
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=10,
        help="Number of role-level pairs to list",
    )
...
def normalize_role_level(value: str) -> str:
    cleaned = SPACE_RE.sub(" ", value.strip())
    return cleaned


def load_role_levels(path: Path) -> dict[str, str]:
    rows = read_rows(path)
    token_to_role_level: dict[str, str] = {}
    for row in rows:
        token = (row.get("token") or "").strip()
        if not token:
            continue
        role_level = normalize_role_level(row.get("role_level") or "")
        if not role_level:
            continue
        token_to_role_level[token] = role_level
    return token_to_role_level
...
def cleaned_rows(
    ...
    for row in rows:
        ...
        if statement_id not in seed_ids:
            continue
        role_level = token_to_role_level.get(token)
        if not role_level:
            continue
...
def compute_role_level_stats(rows: Iterable[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    grouped: dict[str, dict[str, list[Any]]] = defaultdict(lambda: {"x": [], "y": [], "tokens": []})
    for row in rows:
        grouped[row["role_level"]]["x"].append(row["x"])
        grouped[row["role_level"]]["y"].append(row["y"])
        grouped[row["role_level"]]["tokens"].append(row["token"])

    stats: dict[str, dict[str, Any]] = {}
    for role_level, data in grouped.items():
...
def compute_pair_distances(stats: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    role_levels = sorted(stats.keys())
    pairs: list[dict[str, Any]] = []
    for i, role_a in enumerate(role_levels):
        for role_b in role_levels[i + 1 :]:
            a = stats[role_a]
            b = stats[role_b]
...
def summarize(
    ...
    lines = [
        "Q15 Role Level Differences Summary",
        "=================================",
        f"Role levels included: {len(stats)}",
        f"Participants included: {total_participants}",
        f"Placements included: {total_placements}",
        "",
        "Overall tests across role levels",
        "-------------------------------",
    ...
    if pair_distances:
        lines.extend(
            [
                "Top role-level pairs by centroid distance",
                "-----------------------------------------",
            ]
        )
        for pair in sorted(pair_distances, key=lambda item: (-item["distance"], item["role_level_a"]))[:top_n]:
            lines.append(
                f"- {pair['role_level_a']} vs {pair['role_level_b']}: "
                f"distance {pair['distance']:.2f} (Δx {pair['delta_x']:.2f}, Δy {pair['delta_y']:.2f})"
            )
    else:
        lines.append("Not enough role levels to compute pairwise distances.")
...
def write_csv(stats: dict[str, dict[str, Any]], output_path: Path) -> None:
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        for role_level, data in sorted(stats.items(), key=lambda item: (-item[1]["n_participants"], item[0])):
            writer.writerow(
                {
                    "role_level": role_level,
...
def main() -> None:
    ...
    token_to_role_level = load_role_levels(args.participants)
    placements = read_rows(args.input)
    deduped = deduplicate_latest(placements)
    cleaned = cleaned_rows(deduped, seed_ids, token_to_role_level)
    role_level_stats = compute_role_level_stats(cleaned)
    ...
    x_groups = [
        stats["x_values"]
        for stats in role_level_stats.values()
    ...
    pair_distances = compute_pair_distances(role_level_stats)

    write_csv(role_level_stats, args.output_csv)
