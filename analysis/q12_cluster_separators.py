"""Identify statements that separate participant clusters for Q12."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Q12 cluster separator analysis")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("outputs/q12_statement_cluster_gaps.csv"),
        help="Output CSV path",
    )
    parser.add_argument(
        "--output-summary",
        type=Path,
        default=Path("outputs/q12_summary.txt"),
...
def statement_gap_stats(
    ...
    for row in rows:
        ...
        grouped[statement_id][cluster_id]["x"].append(row["x"])
        grouped[statement_id][cluster_id]["y"].append(row["y"])

    results: list[dict[str, Any]] = []
    for statement_id, cluster_data in grouped.items():
        centroids = []
        ...
        results.append(
            {
                ...
                "clusters_with_data": len(centroids),
                "max_gap": max_gap,
                "delta_x": max_x - min_x,
                "delta_y": max_y - min_y,
            }
...
def summarize(stats: list[dict[str, Any]], top_n: int) -> str:
    lines = [
        "Q12 Cluster Separator Summary",
        "==============================",
        f"Statements included: {len(stats)}",
        "",
        "Top separating statements",
        "---------------------------",
    ]
...
def write_csv(path: Path, stats: list[dict[str, Any]]) -> None:
    ...
    fieldnames = ["statement_id", "clusters_with_data", "max_gap", "delta_x", "delta_y"]
...
def main() -> None:
    ...
    cluster_map = {token: idx + 1 for idx, cluster in enumerate(clusters) for token in cluster}

    stats = statement_gap_stats(cleaned, cluster_map)
    ...
    sanity_lines = [
        ...
        f"Participants clustered: {len(cluster_map)}",
        f"Statements with >=2 clusters: {len(stats)}",
    ]
