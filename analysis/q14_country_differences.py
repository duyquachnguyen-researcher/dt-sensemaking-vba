"""Analyze placement differences by participant country for Q14."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Q14 placement differences by participant country")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("outputs/q14_country_differences.csv"),
        help="Output CSV path",
    )
    parser.add_argument(
        "--output-summary",
        type=Path,
        default=Path("outputs/q14_summary.txt"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Minimum placements per country to include in tests",
    )
    parser.add_argument(
        "--top-n",
        type=int,
        default=10,
        help="Number of country pairs to list",
    )
...
def load_countries(path: Path) -> dict[str, str]:
    ...
    for row in rows:
        ...
        if not country:
            continue
        token_to_country[token] = country
    return token_to_country
...
def cleaned_rows(
    ...
    for row in rows:
        ...
        if statement_id not in seed_ids:
            continue
        country = token_to_country.get(token)
        if not country:
            continue
...
def compute_country_stats(rows: Iterable[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    grouped: dict[str, dict[str, list[Any]]] = defaultdict(lambda: {"x": [], "y": [], "tokens": []})
    for row in rows:
        grouped[row["country"]]["x"].append(row["x"])
        grouped[row["country"]]["y"].append(row["y"])
        grouped[row["country"]]["tokens"].append(row["token"])
...
def summarize(
    ...
    lines = [
        "Q14 Country Differences Summary",
        "==============================",
        f"Countries included: {len(stats)}",
        f"Participants included: {total_participants}",
        f"Placements included: {total_placements}",
        "",
        "Overall tests across participant countries",
        "------------------------------------------",
    ...
    if pair_distances:
        lines.extend(
            [
                "Top country pairs by centroid distance",
                "--------------------------------------",
            ]
        )
        for pair in sorted(pair_distances, key=lambda item: (-item["distance"], item["country_a"]))[:top_n]:
            lines.append(
                f"- {pair['country_a']} vs {pair['country_b']}: "
                f"distance {pair['distance']:.2f} (Δx {pair['delta_x']:.2f}, Δy {pair['delta_y']:.2f})"
            )
    else:
        lines.append("Not enough countries to compute pairwise distances.")
...
def write_csv(stats: dict[str, dict[str, Any]], output_path: Path) -> None:
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        for country, data in sorted(stats.items(), key=lambda item: (-item[1]["n_participants"], item[0])):
            writer.writerow(
                {
                    "country": country,
...
def main() -> None:
    ...
    token_to_country = load_countries(args.participants)
    placements = read_rows(args.input)
    deduped = deduplicate_latest(placements)
    cleaned = cleaned_rows(deduped, seed_ids, token_to_country)
    country_stats = compute_country_stats(cleaned)
    ...
    pair_distances = compute_pair_distances(country_stats)

    write_csv(country_stats, args.output_csv)
